...
A reference agentmux Setup: `bin/tray`, the agent list, for the Tray
column. An agentmux instance uses it by saying `setup = ~/code/agentwork`
and `tray = tray` in its config; agentmux (`~/code/agentmux`) is the
